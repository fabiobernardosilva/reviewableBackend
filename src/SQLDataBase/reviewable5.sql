...
BEGIN
select avg(staffRating) as averageStaffRating,avg(facultyRating) as averageFacultyRating,avg(facilityRating)as averageFacilityRating from reviews group by schoolId;
END//
DELIMITER ;
...
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `reviewsBySchool`(IN sID int)
BEGIN 
  SELECT users.UserFullName,reviews.facultyRating,reviews.facilityRating,reviews.staffRating,reviews.comments,reviews.recommendation,reviews.reviewDateTime FROM (reviews left JOIN schools ON reviews.schoolId = schools.schoolId) 

...
-- Dumping structure for procedure r2.check_rating
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `check_rating`(IN rating1  INT, IN rating2 INT, IN rating3 INT)
BEGIN
    IF rating1 >5 or rating1 < 1 THEN
        SIGNAL SQLSTATE '45000'
           SET MESSAGE_TEXT = 'Faculty Rating is not Valid';
    END IF;
    
    IF rating2 >5 or rating2 < 1 THEN
 SIGNAL SQLSTATE '45001'
    SET MESSAGE_TEXT = 'Staff Rating is not Valid';
    END IF;
    
     IF rating3 >5 or rating3 < 1 THEN
 SIGNAL SQLSTATE '45002'
           SET MESSAGE_TEXT = 'Facility Rating is not Valid';
    END IF;
END//
DELIMITER ;
...
DELIMITER //
CREATE DEFINER=`root`@`localhost` EVENT `delemptyrating` ON SCHEDULE EVERY 2 MINUTE STARTS '2019-03-22 04:27:34' ON COMPLETION PRESERVE ENABLE DO BEGIN
...
) ENGINE=InnoDB AUTO_INCREMENT=35 DEFAULT CHARSET=latin1 COMMENT='Master table for reviews';
...
begin
select schoolName As School_Name,  (ROUND(((ROUND(AVG(s.staffRating),2))+ROUND(AVG(s.facultyRating),2)+
(ROUND(AVG(s.facilityRating),2)))/3,2)) as OverAll_SchoolRating 
...
-- Dumping structure for trigger r2.ratingBeforeInsert
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `ratingBeforeInsert` BEFORE INSERT ON `reviews` FOR EACH ROW BEGIN CALL check_rating(new.facultyRating,new.staffRating, new.facilityRating);
END//
DELIMITER ;
SET SQL_MODE=@OLDTMP_SQL_MODE;

-- Dumping structure for trigger r2.ratingBeforeupdate
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `ratingBeforeupdate` BEFORE UPDATE ON `reviews` FOR EACH ROW BEGIN CALL check_rating(new.facultyRating,new.staffRating, new.facilityRating);
